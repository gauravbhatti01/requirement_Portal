...
function AppShell() {
  ...
  const openViewClient = (id: number) => setViewClientId(id);
  ...
  const handleDashboardEditReq = (id: number) => {
    const r = state.requirements.find(x => x.id === id) ?? null;
    setEditReq(r);
    setReqFormOpen(true);
  };
  ...
  return (
    <div className={styles.shell}>
      ...
      <div className={styles.main}>
        ...
        <div className={styles.content}>
          <AnimatePresence mode="wait" initial={false}>
            ...
            >
              {page === 'dashboard' && (
                <Dashboard onEditReq={handleDashboardEditReq} />
              )}
              {page === 'clients' && (
                <ClientsPage
                  onView={openViewClient}
                  onEdit={openEditClient}
                  onAddReqFor={cid => openAddReq(cid)}
                />
              )}
              {page === 'requirements' && (
                <ReqsPage onEdit={openEditReq} />
              )}
              {page === 'kanban' && (
                <KanbanPage onEditReq={openEditReq} />
              )}
              {page === 'analytics' && (
                <AnalyticsPage />
              )}
      ...
      <ClientView
        ...
        onAddReq={() => {
          const cid = viewClientId ?? undefined;
          setViewClientId(null);
          openAddReq(cid);
        }}
