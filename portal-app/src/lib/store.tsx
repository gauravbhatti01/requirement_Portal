  ...
  switch (action.type) {
    case 'ADD_CLIENT': {
      ...
      return {
        ...
        nextCid: state.nextCid + 1,
        activity: [{ msg: `Added client: ${client.name}`, time: new Date().toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit'}), id: state.nextAid }, ...state.activity].slice(0, 40),
        nextAid: state.nextAid + 1,
      };
    }
    case 'UPDATE_CLIENT': {
      return {
        ...
        activity: [{ msg: `Updated client: ${action.payload.name}`, time: new Date().toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit'}), id: state.nextAid }, ...state.activity].slice(0, 40),
        nextAid: state.nextAid + 1,
      };
    }
    case 'DELETE_CLIENT': {
      ...
      return {
        ...
        requirements: state.requirements.filter(x => x.clientId !== action.id),
        activity: [{ msg: `Deleted client: ${c?.name}`, time: new Date().toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit'}), id: state.nextAid }, ...state.activity].slice(0, 40),
        nextAid: state.nextAid + 1,
      };
    }
    case 'ADD_REQ': {
      ...
      return {
        ...
        nextRid: state.nextRid + 1,
        activity: [{ msg: `Added requirement: ${req.title}`, time: new Date().toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit'}), id: state.nextAid }, ...state.activity].slice(0, 40),
        nextAid: state.nextAid + 1,
      };
    }
    case 'UPDATE_REQ': {
      return {
        ...state,
        requirements: state.requirements.map(r => r.id === action.payload.id ? action.payload : r),
        activity: [{ msg: `Updated: ${action.payload.title}`, time: new Date().toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit'}), id: state.nextAid }, ...state.activity].slice(0, 40),
        nextAid: state.nextAid + 1,
      };
    }
    case 'DELETE_REQ': {
      ...
      return {
        ...state,
        requirements: state.requirements.filter(x => x.id !== action.id),
        activity: [{ msg: `Deleted: ${r?.title}`, time: new Date().toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit'}), id: state.nextAid }, ...state.activity].slice(0, 40),
        nextAid: state.nextAid + 1,
      };
    }
    case 'QUICK_STATUS': {
      ...
      return {
        ...state,
        requirements: state.requirements.map(x => x.id === action.id ? { ...x, status: action.status } : x),
        activity: [{ msg: `Status → ${action.status}: ${r?.title}`, time: new Date().toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit'}), id: state.nextAid }, ...state.activity].slice(0, 40),
        nextAid: state.nextAid + 1,
      };
    }
    case 'LOG': {
      return {
        ...state,
        activity: [{ msg: action.msg, time: new Date().toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit'}), id: state.nextAid }, ...state.activity].slice(0, 40),
        nextAid: state.nextAid + 1,
      };
    }
...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  return (
    <AppContext.Provider value={{ state, dispatch, clientById, reqsForClient, isOverdue, daysUntil, fmtDate, initials }}>
      {children}
    </AppContext.Provider>
  );
}
